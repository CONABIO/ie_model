...
(WCS 2019). La integridad más alta la alcanzan las áreas no afectadas de manera 
significativa por actividades humanas, las cuales son fundamentales para la 
conservación de la biodiversidad, pues es en ecosistemas con alta integridad que 
las especies tienen menor riesgo de extinción (DiMarco et al. 2019). Además, 
estas áreas contribuyen en mayor medida, respecto a áreas degradadas, a los 
servicios ecológicos, como la absorción de CO_2, el suministro de agua y 
protección de riesgos causados por el cambio climático (Watson et al. 2018; 
Martin and Watson 2016). Por lo cual, es de gran importancia, medir la IE para 
preservar áreas con alta integridad y rehabilitar áreas de baja integridad.
...
predice un índice de IE. Para esto es necesario definir los valores de IE que 
se tenga en algún momento para después entrenar este modelo con estos valores 
y el conjunto de variables predictoras a un tiempo dado de tal forma que se 
pueda crear la relación de los valores IE con las variables predictoras. Al 
hacer tendremos un modelo el cual a partir de los valores de las variables 
predictoras se obtiene el valor de la IE. Es importante notar que una mejor 
modelación va a depender de la elección que se haga de los parámetros del modelo 
XGBoost y de lo relevante de las variables predictoras para el valor de la IE.

### Índice de Integridad Ecosistémica

La IE es un concepto complejo, por lo que para su medición usamos una medida 
_proxy_ que refleje el impacto humano en la naturaleza (Hansen, A. J 2021). En 
este caso se definió un índice de acuerdo al grado de transformación de la 
...
grandes cambios antropogénicos) (INEGI, 2003), con el mapa de uso de suelo y 
vegetación más reciente (INEGI, 2017). Se asignó un valor al índice de acuerdo 
...
cuatro categorías corresponde a _integridad baja_. De esta forma el Índice de IE 
...
Las variables utilizadas para la predicción del índice de IE provienen de datos 
...
estimación de la IE actual e histórica. 

Para estimar la IE, debemos considerar que México cuenta con distintos tipos de 
...
predicción de la altura del dosel (Nandy 2021). Otra variable incluida para 
...
variables predictoras, así como el del índice de IE, fueron proyectados a la 
misma extención, sistema de coordenadas (epsg) y resolución, obteniendo mapas 
...
específico las bandas VV y VH, así como la productividad primaria bruta media y 
su desviación estándar. Se transformó cada uno de los mapas de variables 
predictoras y el del índice de IE a estos _superpixeles_.

Para relacionar las variables predictoras con el índice de IE, se entrenó un 
modelo de aprendizaje de máquina llamado XGBoost. Con este modelo se obtiene la 
probabilidad de que cada _superpixel_ pertenezca a cada clase del índice de IE, 
asignando el valor con la mayor probabilidad.

Se puede predecir el índice de IE para el periodo y territorio deseado 
utilizando el modelo entrenado con datos del 2017. 
